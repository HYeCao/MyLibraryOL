...
<!--        <th>returnID</th>-->
    ...
    //    $url=$url['path'];
    //		echo($url);
    $url="/bibliosoft/ReaderBNewHistory.php";

    $sql="select * from `returninfor`,`bookinfor`,`reader_table`  where returninfor.bookID=bookinfor.bookID and reader_table.readername='$name'
and returninfor.readerID=reader_table.readerID and bookName like '%$bookName%'";
    $query=mysqli_query($con,$sql);
    $rows=mysqli_num_rows($query);

    $sql5="select * from `borrowinfor`,`bookinfor`,`reader_table`  where borrowinfor.bookID=bookinfor.bookID and reader_table.readername='$name'
and borrowinfor.readername=reader_table.readername and bookName like '%$bookName%'";
//    $query=mysqli_query($con,$sql);
//    $sql5="select * from borrowinfor ";
    ...
    $sqli="select * from `borrowinfor`,`bookinfor`,`reader_table`  where borrowinfor.bookID=bookinfor.bookID and reader_table.readername='$name'
and borrowinfor.readername=reader_table.readername and bookName like '%$bookName%'limit $page,$pagesize";
...
and returninfor.readerID=reader_table.readerID and bookName like '%$bookName%'limit $page,$pagesize";
    $result = mysqli_query($con,$sqli);
    while($row = mysqli_fetch_array($result))
    {
        $id=$row['borrowID'];
